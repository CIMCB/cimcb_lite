...
def scale(x, axis=0, ddof=1, method="auto", mu="default", sigma="default", return_mu_sigma=False):
    """Scales x (which can include nans) with method: 'auto', 'pareto', 'vast', 'level' or 'range'.
    ...
    if method is "auto":
        ...
        z = (x - mu) / (np.nanmax(x, axis=axis) - np.nanmin(x, axis=axis))
    else:
        raise ValueError("Method has to be either 'auto', 'pareto', 'vast', 'level' or 'range'.")
